...
export function CodeEditor({ code, language, onChange, onRun, running, stdout, stderr }: Props) {
  return <div className="editor-wrap">
    <div className="editor-bar"><span className="text-xs text-slate-500">solution.{language === 'python' ? 'py' : 'js'}</span><span className="text-xs text-slate-500">{language}</span></div>
