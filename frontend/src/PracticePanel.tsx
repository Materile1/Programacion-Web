...
function TutorAnswer({ answer, mode }: { answer: string; mode: string }) {
  const simpleMarker = 'En palabras simples:'
  const technicalMarker = 'En términos técnicos:'
  const simpleStart = answer.indexOf(simpleMarker)
  const technicalStart = answer.indexOf(technicalMarker)
  const simple = simpleStart >= 0 ? answer.slice(simpleStart + simpleMarker.length, technicalStart >= 0 ? technicalStart : undefined).trim() : answer
  const technical = technicalStart >= 0 ? answer.slice(technicalStart + technicalMarker.length).trim() : ''
  return <div className="mt-4 space-y-3 text-sm leading-6 text-slate-300"><div className="eyebrow text-mint">{mode === 'ai-configured' ? 'TUTOR IA' : 'TUTOR LOCAL'}</div><div className="rounded-lg border border-[#315144] bg-[#10241e] p-3"><div className="font-semibold text-mint">En palabras simples</div><p className="mt-1 whitespace-pre-line">{simple}</p></div>{technical && <div className="rounded-lg border border-[#31506a] bg-[#0d1d2a] p-3"><div className="font-semibold text-aqua">En términos técnicos</div><p className="mt-1 whitespace-pre-line">{technical}</p></div>}</div>
}
  ...
  return <div className="space-y-7">
    ...
    <div className="grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
      <section className="panel">
        ...
        <div className="mt-8 rounded-xl border border-line bg-[#0a141c] p-4"><div className="flex items-center gap-2 text-xs text-slate-400"><Sparkles size={14} className="text-amber" /> Tutor socrático</div><textarea className="form-input mt-3 min-h-20 w-full resize-y" value={question} onChange={event => setQuestion(event.target.value)} placeholder="¿Por dónde empiezo con este reto?" /><button className="primary-button mt-3" onClick={() => requestTutor(question.trim() || 'No sé por dónde empezar con este reto')} disabled={asking}>{asking ? 'Pensando...' : 'Pedir ayuda'}</button>{tutorError && <p className="mt-3 text-sm text-coral">{tutorError}</p>}{tutor && <TutorAnswer answer={tutor.answer} mode={tutor.mode} />}</div>
